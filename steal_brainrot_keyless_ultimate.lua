-- ╔══════════════════════════════════════════════════════╗
-- ║           MLG HACK v4.0 - ULTIMATE EDITION          ║
-- ║     Skins + Todas Features + Universal              ║
-- ╚══════════════════════════════════════════════════════╝
...
-- ============================================================
-- SKINS
-- Para criar: copie qualquer bloco abaixo, mude nome e cores!
-- ============================================================
local SKINS = {
    {
        name    = "🟢 Matrix",
        bg      = Color3.fromRGB(8, 14, 8),
        topbar  = Color3.fromRGB(5, 20, 5),
        accent  = Color3.fromRGB(0, 255, 80),
        rowBg   = Color3.fromRGB(14, 24, 14),
        text    = Color3.fromRGB(180, 255, 180),
        on      = Color3.fromRGB(0, 220, 70),
        off     = Color3.fromRGB(30, 60, 30),
        rainbow = false,
    },
    {
        name    = "🔵 Ocean",
        bg      = Color3.fromRGB(5, 12, 25),
        topbar  = Color3.fromRGB(5, 10, 22),
        accent  = Color3.fromRGB(0, 180, 255),
        rowBg   = Color3.fromRGB(10, 18, 36),
        text    = Color3.fromRGB(180, 220, 255),
        on      = Color3.fromRGB(0, 160, 255),
        off     = Color3.fromRGB(20, 40, 80),
        rainbow = false,
    },
    {
        name    = "🔴 Blood",
        bg      = Color3.fromRGB(18, 5, 5),
        topbar  = Color3.fromRGB(14, 4, 4),
        accent  = Color3.fromRGB(255, 40, 40),
        rowBg   = Color3.fromRGB(26, 8, 8),
        text    = Color3.fromRGB(255, 180, 180),
        on      = Color3.fromRGB(220, 40, 40),
        off     = Color3.fromRGB(70, 20, 20),
        rainbow = false,
    },
    {
        name    = "🌈 Rainbow",
        ...
        rowBg   = Color3.fromRGB(16, 16, 28),
        text    = Color3.fromRGB(255, 255, 255),
        on      = Color3.fromRGB(100, 255, 200),
        off     = Color3.fromRGB(40, 40, 70),
        rainbow = true,
    },
    {
        name    = "🟡 Gold",
        bg      = Color3.fromRGB(16, 13, 4),
        topbar  = Color3.fromRGB(12, 10, 3),
        accent  = Color3.fromRGB(255, 200, 0),
        rowBg   = Color3.fromRGB(24, 20, 6),
        text    = Color3.fromRGB(255, 240, 180),
        on      = Color3.fromRGB(220, 180, 0),
        off     = Color3.fromRGB(60, 50, 10),
        rainbow = false,
    },
    {
        name    = "⚫ Dark",
        bg      = Color3.fromRGB(6, 6, 6),
        topbar  = Color3.fromRGB(4, 4, 4),
        accent  = Color3.fromRGB(180, 180, 180),
        rowBg   = Color3.fromRGB(14, 14, 14),
        text    = Color3.fromRGB(220, 220, 220),
        on      = Color3.fromRGB(160, 160, 160),
        off     = Color3.fromRGB(40, 40, 40),
        rainbow = false,
    },
    -- ╔══════════════════════════════════════╗
    -- ║  CRIE SUA SKIN AQUI - copie este bloco e mude as cores!
    -- ║  R, G, B = números de 0 a 255
    -- ╚══════════════════════════════════════╝
    -- {
    --     name    = "🎨 MinhaSkin",
    --     bg      = Color3.fromRGB(R, G, B),   -- fundo geral
    --     topbar  = Color3.fromRGB(R, G, B),   -- barra do topo
    --     accent  = Color3.fromRGB(R, G, B),   -- cor principal / borda
    --     rowBg   = Color3.fromRGB(R, G, B),   -- fundo de cada linha
    --     text    = Color3.fromRGB(R, G, B),   -- cor do texto
    --     on      = Color3.fromRGB(R, G, B),   -- cor do toggle ON
    --     off     = Color3.fromRGB(R, G, B),   -- cor do toggle OFF
    --     rainbow = false,                      -- true = borda arco-íris
    -- },
}

local skinIdx = 1
local S = SKINS[skinIdx]  -- skin atual
...
local function notify(msg)
    pcall(StarterGui.SetCore, StarterGui, "SendNotification", {Title="⚡ MLG Hack", Text=msg, Duration=3})
end
...
local function stopFly()
    ...
    local h = getHR(); if h then h.PlatformStand = false end
end
...
local function startGod()
    ...
    CONNS.god = h:GetPropertyChangedSignal("Health"):Connect(function() if F.god then h.Health = math.huge end end)
end

-- INF JUMP
UserInputService.JumpRequest:Connect(function()
    if F.infjump then local h = getHR(); if h then h:ChangeState(Enum.HumanoidStateType.Jumping) end end
end)
...
local function startClickTP()
    CONNS.clicktp = UserInputService.InputBegan:Connect(function(inp, gp)
        ...
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            local ray = workspace:Raycast(camera.CFrame.Position, camera.CFrame.LookVector * 600)
...
RunService.Heartbeat:Connect(function()
    ...
    rbH = (rbH + 0.5) % 360
    local c = lp.Character; if not c then return end
    for _, p in ipairs(c:GetDescendants()) do if p:IsA("BasePart") then p.Color = Color3.fromHSV(rbH/360,1,1) end end
end)
...
local function clearESP()
    for _, h in pairs(espHL) do if h and h.Parent then h:Destroy() end end; espHL = {}
end
local function makeHL(char)
    local h = Instance.new("Highlight")
    h.FillColor = Color3.fromRGB(255,50,50); h.OutlineColor = Color3.fromRGB(255,255,0)
    h.FillTransparency = 0.45; h.Parent = char; return h
end
local function applyESP()
    clearESP()
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= lp and p.Character then
            espHL[p.Name] = makeHL(p.Character)
            p.CharacterAdded:Connect(function(c) if F.esp then espHL[p.Name] = makeHL(c) end end)
        end
    end
end
...
-- FOV
local function applyFOV() camera.FieldOfView = F.fovV end
local function resetFOV()  camera.FieldOfView = 70 end

-- SPEED JUMP
local function applySpeedJump()
    local h = getHR(); if h then h.JumpPower = F.speedjump and 120 or 50 end
end
...
local function stopFreecam()
    disc("freecam"); if fcBV then fcBV:Destroy() end
end

-- ANTI AFK
local vu = Instance.new("VirtualUser"); vu.Parent = game
CONNS.afk = lp.Idled:Connect(function()
    if F.antiafk then vu:CaptureController(); vu:ClickButton2(Vector2.new()) end
end)

-- RESPAWN
lp.CharacterAdded:Connect(function(c)
    c:WaitForChild("Humanoid", 5); c:WaitForChild("HumanoidRootPart", 5)
    task.wait(0.5)
...
WIN.Name = "WIN"
WIN.Size = UDim2.new(0, 460, 0, 620)
WIN.Position = UDim2.new(0.5, -230, 0.5, -310)
WIN.BackgroundColor3 = S.bg
...
STROKE.Thickness = 2; STROKE.Color = S.accent; STROKE.Parent = WIN

-- Rainbow borda
RunService.Heartbeat:Connect(function()
    if not S.rainbow then return end
    strokeH = (strokeH + 0.5) % 360
    STROKE.Color = Color3.fromHSV(strokeH/360, 1, 1)
end)

-- ── TOPBAR ────────────────────────────────────────────────────
local TOP = Instance.new("Frame")
TOP.Size = UDim2.new(1,0,0,60); TOP.BackgroundColor3 = S.topbar
TOP.BorderSizePixel = 0; TOP.ZIndex = 5; TOP.Parent = WIN
...
SUBLBL.Size = UDim2.new(0, 250, 0, 16); SUBLBL.Position = UDim2.new(0, 14, 0, 40)
SUBLBL.BackgroundTransparency = 1; SUBLBL.Text = "v4.0  ·  ULTIMATE  ·  UNIVERSAL"
...
local function mkBtn(t, col, xo)
    ...
    b.Size = UDim2.new(0,32,0,32); b.Position = UDim2.new(1,xo,0,14)
...
MINIBTN.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(WIN, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size=tgt}):Play()
end)
...
TBTNS["move"].BackgroundColor3 = S.accent
TBTNS["move"].TextColor3 = Color3.new(1,1,1)
...
SLBL.Text = "✅ MLG Hack v4 carregado!  |  Insert=esconder  |  F2=Fly  |  F3=Noclip"
...
local function setStatus(t) SLBL.Text = "  " .. t end

-- ── COMPONENTE TOGGLE ROW ─────────────────────────────────────
local allRows = {}  -- para reskinar

local function mkRow(parent, icon, name, flagKey, onToggle, numKey, numDefault, onNum)
    ...
    if numKey then
        ...
        box.BackgroundColor3 = Color3.fromRGB(18,18,28); box.Text = tostring(numDefault)
        box.TextColor3 = S.accent; box.TextScaled = true; box.Font = Enum.Font.GothamBold; box.ZIndex = 5; box.Parent = row
        Instance.new("UICorner", box).CornerRadius = UDim.new(0,6)
        box.FocusLost:Connect(function()
            local n = tonumber(box.Text); if n and onNum then onNum(n); setStatus("✏️ "..name.." = "..n) end
        end)
    end
    ...
    tog.MouseButton1Click:Connect(function()
        local st = onToggle()
        TweenService:Create(pill, TweenInfo.new(0.18), {BackgroundColor3 = st and S.on or S.off}):Play()
        TweenService:Create(dot,  TweenInfo.new(0.18), {Position = st and UDim2.new(1,-23,0.5,-10) or UDim2.new(0,3,0.5,-10)}):Play()
        setStatus((st and "✅" or "❌").." "..name)
    end)

    row.MouseEnter:Connect(function() TweenService:Create(row,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(26,26,42)}):Play() end)
    row.MouseLeave:Connect(function() TweenService:Create(row,TweenInfo.new(0.1),{BackgroundColor3=S.rowBg}):Play() end)
end

-- ── MOVIMENTO ─────────────────────────────────────────────────
mkRow(pgMove, "⚡","Speed Hack","speed", function() F.speed=not F.speed; return F.speed end,
    "speedV", F.speedV, function(v) F.speedV=math.max(16,v) end)

mkRow(pgMove, "🦅","Fly Mode","fly", function()
    F.fly=not F.fly; if F.fly then startFly() else stopFly() end; return F.fly end,
    "flyV", F.flyV, function(v) F.flyV=math.max(10,v) end)

mkRow(pgMove, "👻","Noclip","noclip", function()
    F.noclip=not F.noclip; if F.noclip then startNoclip() else stopNoclip() end; return F.noclip end)

mkRow(pgMove, "♾️","Infinite Jump","infjump", function() F.infjump=not F.infjump; return F.infjump end)

mkRow(pgMove, "🖱️","Click Teleport","clicktp", function()
    F.clicktp=not F.clicktp; if F.clicktp then startClickTP() else disc("clicktp") end; return F.clicktp end)

mkRow(pgMove, "🌀","Spin Mode","spin", function() F.spin=not F.spin; return F.spin end)

mkRow(pgMove, "🎥","Free Cam (I/J/K/L)","freecam", function()
    F.freecam=not F.freecam; if F.freecam then startFreecam() else stopFreecam() end; return F.freecam end)

mkRow(pgMove, "🛑","Anti AFK","antiafk", function() F.antiafk=not F.antiafk; return F.antiafk end)

-- ── COMBAT ────────────────────────────────────────────────────
mkRow(pgCombat, "🛡️","God Mode","god", function()
    F.god=not F.god; if F.god then startGod() else disc("god") end; return F.god end)

mkRow(pgCombat, "📦","Hitbox Expander","hitbox", function()
    F.hitbox=not F.hitbox; if F.hitbox then applyHitbox() end; return F.hitbox end,
    "hitboxV", F.hitboxV, function(v) F.hitboxV=math.max(1,v); if F.hitbox then applyHitbox() end end)

mkRow(pgCombat, "🗡️","Reach Hack","reach", function()
    F.reach=not F.reach; return F.reach end,
    "reachV", F.reachV, function(v) F.reachV=math.max(1,v) end)

mkRow(pgCombat, "🦘","Speed Jump","speedjump", function()
    F.speedjump=not F.speedjump; applySpeedJump(); return F.speedjump end)

mkRow(pgCombat, "🌑","Low Gravity","lowgrav", function()
    F.lowgrav=not F.lowgrav; workspace.Gravity=F.lowgrav and 40 or 196.2; return F.lowgrav end)

-- ── VISUAL ────────────────────────────────────────────────────
mkRow(pgVisual, "👁️","ESP Players","esp", function()
    F.esp=not F.esp; if F.esp then applyESP() else clearESP() end; return F.esp end)

mkRow(pgVisual, "🌈","Rainbow Char","rainbow", function()
    F.rainbow=not F.rainbow
    if not F.rainbow then
    ...
    end; return F.rainbow end)

mkRow(pgVisual, "🔦","Fullbright","fullbright", function()
    F.fullbright=not F.fullbright; if F.fullbright then startFB() else stopFB() end; return F.fullbright end)

mkRow(pgVisual, "🔭","FOV Changer","fov", function()
    F.fov=not F.fov; if F.fov then applyFOV() else resetFOV() end; return F.fov end,
...
local function mkWorldBox(parent, label, default, onApply, onReset)
    ...
    ap.MouseButton1Click:Connect(function() local n = tonumber(bx.Text); if n then onApply(n) end end)
    if onReset then
        ...
        rs.MouseButton1Click:Connect(onReset)
    end
end
...
-- Explodir todos os objetos físicos
local explBtn = Instance.new("TextButton"); explBtn.Size = UDim2.new(1,-8,0,42); explBtn.BackgroundColor3 = S.rowBg
explBtn.Text = "💥  Explodir Mapa (físico)"; explBtn.TextColor3 = S.text; explBtn.TextScaled = true; explBtn.Font = Enum.Font.GothamBold; explBtn.ZIndex = 5; explBtn.Parent = pgWorld
...
explBtn.MouseButton1Click:Connect(function()
    local _, r = getHR(); if not r then return end
    local e = Instance.new("Explosion"); e.BlastRadius = 60; e.BlastPressure = 5e5
    e.Position = r.Position; e.Parent = workspace
...
tpLbl.BackgroundTransparency = 1; tpLbl.Text = "🚀 Teleportar para jogador:"; tpLbl.TextColor3 = S.text; tpLbl.TextXAlignment = Enum.TextXAlignment.Left; tpLbl.TextScaled = true; tpLbl.Font = Enum.Font.GothamBold; tpLbl.ZIndex = 5; tpLbl.Parent = tpRow
local tpBox = Instance.new("TextBox"); tpBox.Size = UDim2.new(0,120,0,22); tpBox.Position = UDim2.new(0,10,0,30)
tpBox.BackgroundColor3 = Color3.fromRGB(18,18,28); tpBox.Text = "Nome"; tpBox.TextColor3 = S.accent; tpBox.TextScaled = true; tpBox.Font = Enum.Font.GothamBold; tpBox.ZIndex = 5; tpBox.Parent = tpRow
...
skinHeader.Text = "  🎨 Escolha sua skin:"; skinHeader.TextColor3 = S.accent; skinHeader.TextXAlignment = Enum.TextXAlignment.Left
skinHeader.TextScaled = true; skinHeader.Font = Enum.Font.GothamBold; skinHeader.ZIndex = 5; skinHeader.Parent = pgSkins

local function applyAllSkin(sk)
    S = sk
    WIN.BackgroundColor3 = sk.bg
    TOP.BackgroundColor3 = sk.topbar
    topFix.BackgroundColor3 = sk.topbar
    STROKE.Color = sk.accent
    SLBL.TextColor3 = sk.accent
    skinHeader.TextColor3 = sk.accent
    TG.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, sk.accent), ColorSequenceKeypoint.new(1, Color3.fromRGB(0,0,0))}
    for _, row in ipairs(allRows) do row.BackgroundColor3 = sk.rowBg end
    for _, pg in pairs(PAGES) do pg.ScrollBarImageColor3 = sk.accent end
    -- botões de tab ativos
    for id, btn in pairs(TBTNS) do
        if PAGES[id].Visible then btn.BackgroundColor3 = sk.accent end
    end
end
...
for i, sk in ipairs(SKINS) do
    ...
    -- preview de 4 cores
    local previewColors = {sk.accent, sk.on, sk.rowBg, sk.text}
    for j, col in ipairs(previewColors) do
        local pd = Instance.new("Frame"); pd.Size = UDim2.new(0,16,0,16)
        pd.Position = UDim2.new(0.57+(j-1)*0.1, 0, 0.5, -8)
    ...
    selBtn.MouseButton1Click:Connect(function()
        skinIdx = i
        applyAllSkin(sk)
        setStatus("🎨 Skin: "..sk.name)
        notify("Skin aplicada: "..sk.name)
    end)

    selBtn.MouseEnter:Connect(function() TweenService:Create(sb,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(30,30,50)}):Play() end)
    selBtn.MouseLeave:Connect(function() TweenService:Create(sb,TweenInfo.new(0.1),{BackgroundColor3=sk.rowBg}):Play() end)
end

-- Guia criar skin
local guide = Instance.new("Frame"); guide.Size = UDim2.new(1,-8,0,90); guide.BackgroundColor3 = Color3.fromRGB(16,16,28); guide.BorderSizePixel = 0; guide.Parent = pgSkins
...
gl.Text = "💡 CRIAR SUA SKIN:\nNo script, role até encontrar\n\"CRIE SUA SKIN AQUI\"\nCopie o bloco, troque os valores\nRGB (0-255) e dê um nome!"
...
UserInputService.InputBegan:Connect(function(inp, gp)
    ...
    if inp.KeyCode == Enum.KeyCode.Insert then
        guiOpen = not guiOpen
        if guiOpen then WIN.Visible=true; TweenService:Create(WIN,TweenInfo.new(0.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,460,0,620)}):Play()
        else TweenService:Create(WIN,TweenInfo.new(0.2),{Size=UDim2.new(0,0,0,0)}):Play(); task.delay(0.25,function() WIN.Visible=false end) end
    end
...
-- ── PRONTO ────────────────────────────────────────────────────
notify("⚡ MLG Hack v4.0 ULTIMATE carregado!")
print("╔════════════════════════════════════╗")
print("║  ⚡  MLG HACK v4.0 ULTIMATE EDITION ║")
print("║  Insert=GUI | F2=Fly | F3=Noclip   ║")
print("║  F4=ESP | P=+vel | Ç=-vel          ║")
print("║  FreeCam: I/J/K/L                  ║")
print("╚════════════════════════════════════╝")
